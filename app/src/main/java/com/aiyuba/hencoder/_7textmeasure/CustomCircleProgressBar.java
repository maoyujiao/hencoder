...
public class CustomCircleProgressBar extends View {
    ...
    public CustomCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        outsideRadius = a.getDimension(R.styleable.CustomCircleProgressBar_outside_radius, DimenUtil.dp2px(getContext(), 40.0f));
        ...
        direction = a.getInt(R.styleable.CustomCircleProgressBar_direction, 0);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < progressGroup.length; i++) {
            float radius = outsideRadius + i * 25;
            //第一步:画背景(即内层圆)
            ...
            RectF oval = new RectF(circlePoint - radius, circlePoint - radius, circlePoint + radius, circlePoint + radius);  //用于定义的圆弧的形状和大小的界限
            canvas.drawArc(oval, DirectionEnum.getDegree(direction), 360 * (progressCurrent[i] / maxProgress), false, paint);  //根据进度画圆弧
        ...
        for (int i = 0; i < progressGroup.length; i++) {
            ...
            int height = circlePoint;
            for (int j = 0; j < progressStr[i].length(); j++) {
                paint.getTextBounds(progressStr[i].substring(j, j + 1), 0, 1, rect);
                height = height + 25;
                canvas.drawText(progressStr[i].substring(j, j + 1), circlePoint - radius - progressWidth / 2, height, paint);
            }
        }
    ...
    public void setProgressGroup(int[] progress) {
        progressGroup = progress;
        progressCurrent = new float[progressGroup.length];
        startAnim();
    }
    ...
    private void startAnim() {
        for(int k = 0;k < progressGroup.length;k++) {
            final int j = k;
            ValueAnimator animator = ObjectAnimator.ofFloat(0, progressGroup[j]);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    progressCurrent[j] = (float) animation.getAnimatedValue();
                    postInvalidate();
                }
            });
